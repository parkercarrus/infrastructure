...
export default async function PairsStrategyPage() {
  ...
  return (
    <main className="min-h-screen bg-white text-zinc-900 font-mono">
      <StrategyClient
        ...
        trades={trades}
        metrics={metrics}
        description={"A pairs strategy is a market-neutral trading approach that looks for two historically related assets whose prices tend to move together over time. The core idea is to identify a stable relationship (often modeled through correlation, cointegration, or a regression hedge ratio) and then trade deviations from that relationship when the “spread” between them becomes unusually wide or narrow. When the spread diverges, the strategy typically goes long the relatively underpriced asset and short the relatively overpriced one, aiming to profit as the spread reverts back toward its typical level. Risk controls often include position sizing based on volatility, constraints to keep the portfolio close to dollar- or beta-neutral, and exit rules based on spread normalization or stop-loss thresholds if the relationship breaks down. This strategy is currently inactive and replaced by a structured randomness algorithm."}
      />
    </main>
  );
}